 ...
 * 添加 @Params 注解
 */
public class SelectInternal implements IntentionAction, Iconable {
    XmlFile xmlFile;
    ...
    public void invoke(@NotNull Project project, Editor editor, PsiFile file) throws IncorrectOperationException {
        ...
        XmlTag select = xmlFile.getRootTag().createChildTag("select", null, "\n\n\t", false);
        select.setAttribute("id", psiMethod.getName());
        String returnType = psiMethod.getReturnType().getCanonicalText();
        // 获取返回值的全限定类名
        String paramsText = psiMethod.getParameterList().getParameter(0).getType().getPresentableText();
        XmlTag resultMap = XmlFileUtil.getResultMap(xmlFile);
        if (StrUtil.isNotEmpty(paramsText)) {
            select.setAttribute("parameterType", paramsText);
        }
        ...
        WriteCommandAction.runWriteCommandAction(project, () -> {
            PsiElement element = xmlFile.getRootTag().add(select);
    ...
    public @NotNull IntentionPreviewInfo generatePreview(@NotNull Project project, @NotNull Editor editor, @NotNull PsiFile file) {
        ...
        IntentionPreviewInfo diff = new IntentionPreviewInfo.CustomDiff(XmlFileType.INSTANCE,
                description,
                StrUtil.format(CODE, psiMethod.getName(), returnType, paramsText));
        return diff;
    }
